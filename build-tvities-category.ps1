...
$CategoryTitle = 'TVities'
$CategorySlug = 'tvities'

$ImageRoot = Join-Path $RepoRoot 'images\Sketches\TVities'
...
function ConvertTo-TitleCaseSimple {
    ...
    for ($i = 0; $i -lt $words.Count; $i++) {
        ...
        if ($word -match '^(?i)TVities$') {
            $out.Add('TVities')
        }
        elseif ($word -match '^(?i)TVity$') {
            $out.Add('TVity')
        }
        elseif ($word -match '^(?i)(B&W|MC|TV|DNA|PDF|PNG|JPG|JPEG|LCCC|R2|FLAT|AI|M\.U\.S\.C\.L\.E\.?)$') {
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-tvities-build-$timestamp$ext"
...
@"
      ...
      <a class="nav-pill" href="index.html">TVities</a>
"@
    }
    else {
@"
...
<head>
  ...
  <title>$safeTitle | TVities | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to TVities</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>TVities | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <header class="page-header">
      <h1 class="page-title">TVities</h1>
    </header>

    <section class="index-grid" aria-label="TVities sketch and drawing pages">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $heroFile = $null
    $preferred = @(
        'Super Stellar Vision COLOR.jpg',
        'Super Stellar Vision ORIGINAL.jpg',
        'TV Tom COLOR.jpg',
        'TV Tom ORIGINAL COLOR.jpg',
        'TV Superman 2 COLOR.jpg',
        'TVity Messiah COLOR.jpg'
    )
    ...
    if ($null -eq $heroFile) {
        Write-Warning "Could not find a hero image for the TVities card on Sketches index."
        return
    }
    ...
    $newCard = @"
      <a class="gallery-item" href="tvities/index.html">
        <img src="$heroSrc" alt="TVities drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">TVities</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="tvities/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing TVities card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added TVities card to Sketches index."
    }
    else {
        Write-Warning "Could not find an index/gallery grid on Sketches index. TVities page was built, but the main Sketches index was not updated."
    }
...
Write-Host ""
Write-Host "Done. TVities category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\tvities\index.html"
Write-Host ""
Write-Host "Then click several individual TVities pages to check images and titles."
