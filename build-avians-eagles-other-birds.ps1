# build-avians-eagles-other-birds.ps1
...
function Build-CharacterPage {
  ...
  if ($Page.Originals.Count -gt 0) {
    $heading = 'Original Drawing'
    if ($Page.Originals.Count -gt 1) { $heading = 'Original Drawings' }

    [void]$html.AppendLine('    <section class="section-card" aria-labelledby="original-drawings">')
    [void]$html.AppendLine('      <h2 id="original-drawings">' + $heading + '</h2>')
    [void]$html.AppendLine('')
  ...
  [void]$html.AppendLine('      <a href="index.html">Back to ' + (HtmlEncode $CategoryIndexLabel) + '</a>')
  [void]$html.AppendLine('    </div>')
...
function Build-AviansIndexPage {
  $ducksHero = Join-Path $AviansImageRoot $HeroFiles.Ducks
  $eaglesHero = Join-Path $AviansImageRoot $HeroFiles.Eagles
  $otherBirdsHero = Join-Path $AviansImageRoot $HeroFiles.OtherBirds
...
Write-Host ' - The main Avians index now includes Eagles and Other Birds.'
Write-Host ' - The Duck hero path was updated to images\Sketches\avians\Duck HERO.png.'
Write-Host ' - The Eagle and Other Birds hero paths use the exact filenames you provided.'
Write-Host ' - Buzzards was preserved with the existing hero path from the uploaded index.'
Write-Host ''
