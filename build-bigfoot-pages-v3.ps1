...
function Convert-ToWebPath {
  ...
  # Keep URI encoding intact so special filename characters work in the browser.
  # This is especially important for collector-card files containing #.
  # Example: "series 1 #52..." must become "series%201%20%2352..."
...
$csvPath = Join-Path $ReportDir 'bigfoot-build-report-v3.csv'
$txtPath = Join-Path $ReportDir 'bigfoot-build-summary-v3.txt'

$reportRows | Export-Csv -Path $csvPath -NoTypeInformation -Encoding UTF8

$summary = @()
$summary += 'Bigfoot build report V3'
...
Write-Host ''
Write-Host 'Bigfoot pages V3 built successfully.' -ForegroundColor Green
Write-Host ''
...
Write-Host " - $txtPath"
Write-Host ''
Write-Host 'V3 updates applied:' -ForegroundColor Yellow
...
Write-Host ' - Trading-card front/back images now display two per row, with additional no-info card centered below.'
Write-Host ' - URL encoding is preserved so filenames containing # load correctly.'
Write-Host ''
