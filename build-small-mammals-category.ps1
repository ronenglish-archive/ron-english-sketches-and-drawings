...
$CategoryTitle = 'Small Mammals'
$CategorySlug = 'small-mammals'

$ImageCategoryFolderName = 'Small Mammals'
...
$Subcategories = @(
    [pscustomobject]@{ Title='Bats'; Slug='bats'; ImageFolder='bats' },
    [pscustomobject]@{ Title='Fact Ferrets'; Slug='fact-ferrets'; ImageFolder='fact ferrets' },
    [pscustomobject]@{ Title='Groundhogs'; Slug='groundhogs'; ImageFolder='groundhogs' },
    [pscustomobject]@{ Title='Mice'; Slug='mice'; ImageFolder='mice' },
    [pscustomobject]@{ Title='Rabbits'; Slug='rabbits'; ImageFolder='rabbbits' },
    [pscustomobject]@{ Title='Raccoons'; Slug='raccoons'; ImageFolder='raccoons' },
    [pscustomobject]@{ Title='Skunks'; Slug='skunks'; ImageFolder='skunks' },
    [pscustomobject]@{ Title='Squirrels'; Slug='squirrels'; ImageFolder='Squirrels' }
)
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-small-mammals-build-$timestamp$ext"
...
function Get-BaseTitleFromFileName {
    ...
    while ($changed) {
        ...
        # Keep meaningful title words when possible; only strip common view words at the end.
        $s = $s -replace '(?i)\s+three quarters with glasses$', ''
        $s = $s -replace '(?i)\s+three quarters no glasses$', ''
...
function Select-ThumbFile {
    ...
    if ($SubcategoryTitle -eq 'Bats') {
        $categoryPreferredPatterns = @('(?i)^Bats Family Icon COLOR', '(?i)^Bat Eye COLOR', '(?i)^Batcat ORIGINAL')
    }
    elseif ($SubcategoryTitle -eq 'Fact Ferrets') {
        $categoryPreferredPatterns = @('(?i)^Fact Ferret Family Icon COLOR', '(?i)^Farrah Fact Ferret COLOR', '(?i)^Fun Fact Ferret COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Groundhogs') {
        $categoryPreferredPatterns = @('(?i)^Groundhog 1 COLOR', '(?i)^Groundhog 2 COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Mice') {
        $categoryPreferredPatterns = @('(?i)^Mice Logo COLOR', '(?i)^Mousemask Murphy COLOR', '(?i)^Mickeye COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Rabbits') {
        $categoryPreferredPatterns = @('(?i)^Rabbits Family Icon ORIGINAL COLOR', '(?i)^Rabbits Icon ORIGINAL', '(?i)^Ronnie Rabbbit COLOR', '(?i)^Bunny Rabbit COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Raccoons') {
        $categoryPreferredPatterns = @('(?i)^Raccoons Family Icon COLOR', '(?i)^Robbin.*Raccoon COLOR', '(?i)^Rocky Roadkill COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Skunks') {
        $categoryPreferredPatterns = @('(?i)^Skunk Family Icon COLOR', '(?i)^Punk Skunk COLOR', '(?i)^Pink Skunk COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Squirrels') {
        $categoryPreferredPatterns = @('(?i)^Squirrel Family Icon COLOR', '(?i)^Nutjob COLOR', '(?i)^Squirrel Star COLOR')
    }
...
<head>
  ...
  <title>$($Group.Title) | $($Subcategory.Title) | Small Mammals | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    <nav class="nav-pills" aria-label="Site navigation">
      ...
      <a class="nav-pill" href="../../index.html">Small Mammals</a>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>$($Subcategory.Title) | Small Mammals | Ron English Catalogue Raisonné</title>
...
$cardsHtml
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to Small Mammals</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Small Mammals | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    ...
    <header class="page-header">
      <h1 class="page-title">Small Mammals</h1>
    </header>

    <section class="index-grid" aria-label="Small Mammals subcategories">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $preferred = @(
        [pscustomobject]@{ Sub='Mice'; Name='Mice Logo COLOR.jpg' },
        [pscustomobject]@{ Sub='Rabbits'; Name='Rabbits Family Icon ORIGINAL COLOR.jpg' },
        [pscustomobject]@{ Sub='Squirrels'; Name='Squirrel Family Icon COLOR.jpg' },
        [pscustomobject]@{ Sub='Bats'; Name='Bats Family Icon COLOR.jpg' }
    )
    ...
    $newCard = @"
      <a class="gallery-item" href="small-mammals/index.html">
        <img src="$heroSrc" alt="Small Mammals drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">Small Mammals</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="small-mammals/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing Small Mammals card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added Small Mammals card to Sketches index."
    }
    else {
...
Write-Host ""
Write-Host "Done. Small Mammals category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\index.html"
Write-Host ""
Write-Host "Then check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\bats\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\fact-ferrets\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\groundhogs\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\mice\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\rabbits\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\raccoons\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\skunks\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\small-mammals\squirrels\index.html"
Write-Host ""
Write-Host "The script also tried to add/update the Small Mammals card on:"
