...
$CategoryTitle = 'Reptiles'
$CategorySlug = 'reptiles'

$ImageCategoryFolderName = 'Reptiles'
...
$Subcategories = @(
    [pscustomobject]@{ Title='Alligators'; Slug='alligators'; ImageFolder='alligators' },
    [pscustomobject]@{ Title='Chameleons'; Slug='chameleons'; ImageFolder='Chameleons' },
    [pscustomobject]@{ Title='Dinosaurs'; Slug='dinosaurs'; ImageFolder='Dinosaurs' },
    [pscustomobject]@{ Title='Poultry Rex'; Slug='poultry-rex'; ImageFolder='Poultry Rex' },
    [pscustomobject]@{ Title='Snakes'; Slug='snakes'; ImageFolder='Snakes' },
    [pscustomobject]@{ Title='Turtles'; Slug='turtles'; ImageFolder='Turtles' }
)
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-reptiles-build-$timestamp$ext"
...
function Get-BaseTitleFromFileName {
    ...
    while ($changed) {
        ...
        $s = $s -replace '(?i)\s+ORIGINAL\s+\d+x\d+\s*in$', ''
        ...
        $s = $s -replace '(?i)\s+carrying shell$', ''
        $s = $s -replace '(?i)\s+with microphone$', ''
        $s = $s -replace '(?i)\s+smoking$', ''
        $s = $s -replace '(?i)\s+driving$', ''
        $s = $s -replace '(?i)\s+side driving$', ''
...
function Select-ThumbFile {
    ...
    if ($SubcategoryTitle -eq 'Alligators') {
        $categoryPreferredPatterns = @('(?i)^Croc Wild ORIGINAL COLOR', '(?i)^Croc Wild ORIGINAL')
    }
    elseif ($SubcategoryTitle -eq 'Chameleons') {
        $categoryPreferredPatterns = @('(?i)^Paulpop Chameleon COLOR', '(?i)^Paulpop Chameleon front COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Dinosaurs') {
        $categoryPreferredPatterns = @('(?i)^Dinosaur Senior COLOR', '(?i)^Splatter Saurus COLOR', '(?i)^Triceraturtle COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Poultry Rex') {
        $categoryPreferredPatterns = @('(?i)^Poultry Rex Family Icon COLOR', '(?i)^Terrible T COLOR', '(?i)^T Cell COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Snakes') {
        $categoryPreferredPatterns = @('(?i)^Original Gin COLOR', '(?i)^snake color1', '(?i)^Snake 1 ORIGINAL')
    }
    elseif ($SubcategoryTitle -eq 'Turtles') {
        $categoryPreferredPatterns = @('(?i)^Turtle Family Icon COLOR', '(?i)^Slo Moe front COLOR', '(?i)^Tupac Turtle front COLOR')
    }
...
<head>
  ...
  <title>$($Group.Title) | $($Subcategory.Title) | Reptiles | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    <nav class="nav-pills" aria-label="Site navigation">
      ...
      <a class="nav-pill" href="../../index.html">Reptiles</a>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>$($Subcategory.Title) | Reptiles | Ron English Catalogue Raisonné</title>
...
$cardsHtml
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to Reptiles</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Reptiles | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    ...
    <section class="index-grid" aria-label="Reptiles subcategories">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $preferred = @(
        [pscustomobject]@{ Sub='Turtles'; Name='Turtle Family Icon COLOR.jpg' },
        [pscustomobject]@{ Sub='Poultry Rex'; Name='Poultry Rex Family Icon COLOR.jpg' },
        [pscustomobject]@{ Sub='Dinosaurs'; Name='Dinosaur Senior COLOR 2.jpg' },
        [pscustomobject]@{ Sub='Alligators'; Name='Croc Wild ORIGINAL COLOR.jpg' }
    )
    ...
    $newCard = @"
      <a class="gallery-item" href="reptiles/index.html">
        <img src="$heroSrc" alt="Reptiles drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">Reptiles</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="reptiles/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing Reptiles card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added Reptiles card to Sketches index."
    }
    else {
...
Write-Host ""
Write-Host "Done. Reptiles category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\index.html"
Write-Host ""
Write-Host "Then check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\alligators\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\chameleons\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\dinosaurs\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\poultry-rex\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\snakes\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\reptiles\turtles\index.html"
Write-Host ""
Write-Host "The script also tried to add/update the Reptiles card on:"
