...
$CategoryTitle = 'Yin Yangs'
$CategorySlug = 'yin-yangs'

$ImageRoot = Join-Path $RepoRoot 'images\Sketches\Yin Yangs'
...
function ConvertTo-TitleCaseSimple {
    ...
    $clean = $clean -replace '(?i)\byinyang\b', 'Yin Yang'
    ...
    for ($i = 0; $i -lt $words.Count; $i++) {
        ...
        if ($word -match '^(?i)(B&W|MC|TV|DNA|PDF|PNG|JPG|JPEG|LCCC|R2|FLAT|M\.U\.S\.C\.L\.E\.?)$') {
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-yin-yangs-build-$timestamp$ext"
...
function Get-BaseTitleFromFileName {
    ...
    $s = $s -replace '(?i)\byinyang\b', 'Yin Yang'
    $s = $s -replace '\s+', ' '
...
function Select-ThumbFile {
    ...
    $rules = @(
        { param($f) $f.Name -match '(?i)\bCOLOR\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear|bottom)\b' -and $f.Name -notmatch '(?i)colorway' },
        { param($f) $f.Name -match '(?i)\bORIGINAL COLOR\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear|bottom)\b' },
        { param($f) $f.Name -match '(?i)\bORIGINAL\b|\borig\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear|bottom)\b' },
...
function New-BreadcrumbHtml {
    ...
    $up = Get-UpPrefixFromHtmlFolder -HtmlFolder $CurrentHtmlFolder
...
@"
      ...
      <a class="nav-pill" href="index.html">Yin Yangs</a>
"@
    }
    else {
@"
...
<head>
  ...
  <title>$safeTitle | Yin Yangs | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to Yin Yangs</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Yin Yangs | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <header class="page-header">
      <h1 class="page-title">Yin Yangs</h1>
    </header>

    <section class="index-grid" aria-label="Yin Yangs sketch and drawing pages">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $heroFile = $null
    $preferred = @(
        'Yin and Yangs COLOR.jpg',
        'Yin and Yangs ORIGINAL COLOR.jpg',
        'Yin Yang Plush COLOR.jpg',
        'Yanger COLOR.jpg',
        'Big Yang back COLOR.jpg'
    )
    ...
    if ($null -eq $heroFile) {
        Write-Warning "Could not find a hero image for the Yin Yangs card on Sketches index."
        return
    }
    ...
    $newCard = @"
      <a class="gallery-item" href="yin-yangs/index.html">
        <img src="$heroSrc" alt="Yin Yangs drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">Yin Yangs</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="yin-yangs/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing Yin Yangs card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added Yin Yangs card to Sketches index."
    }
    else {
        Write-Warning "Could not find an index/gallery grid on Sketches index. Yin Yangs page was built, but the main Sketches index was not updated."
    }
...
Write-Host ""
Write-Host "Done. Yin Yangs category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\yin-yangs\index.html"
Write-Host ""
Write-Host "Then click several individual Yin Yangs pages to check images and titles."
