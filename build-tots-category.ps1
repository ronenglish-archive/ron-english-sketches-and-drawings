...
$ImageRoot = Join-Path $RepoRoot 'images\Sketches\Tots'
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-tots-build-$timestamp$ext"
...
function Get-BaseTitleFromFileName {
    ...
    # Specific grouping rules for this folder.
    if ($s -match '^(?i)Temper Tot\b') {
        return 'Temper Tot'
    }
    if ($s -match '^(?i)Tantrum Tot\b') {
        return 'Tantrum Tot'
    }
    ...
    while ($changed) {
        ...
        $s = $s -replace '(?i)\s+diaper$', ''
        $s = $s -replace '(?i)\s+segmented$', ''
        $s = $s -replace '(?i)\s+icon$', ''
...
function Select-ThumbFile {
    ...
    $rules = @(
        ...
        { param($f) $f.Name -match '(?i)\bCOLORED\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear|bottom|top)\b' },
        ...
        { param($f) $f.Name -match '(?i)\bCOLOR\b|\bCOLORED\b' },
...
@"
      ...
      <a class="nav-pill" href="index.html">Tots</a>
"@
    }
    else {
@"
...
<head>
  ...
  <title>$safeTitle | Tots | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to Tots</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Tots | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
$breadcrumbs
    ...
    <header class="page-header">
      <h1 class="page-title">Tots</h1>
    </header>

    <section class="index-grid" aria-label="Tots sketch and drawing pages">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $heroFile = $null
    $preferred = @(
        'Temper Tot COLOR.jpg',
        'Temper Tot ORIGINAL.jpg',
        'Tot Toddler COLOR.jpg',
        "Li'l Tot COLOR.jpg",
        'Texas Tantrum Tot COLOR.jpg',
        'Billboard Baby ORIGINAL.jpg'
    )
    ...
    if ($null -eq $heroFile) {
        Write-Warning "Could not find a hero image for the Tots card on Sketches index."
        return
    }
    ...
    $newCard = @"
      <a class="gallery-item" href="tots/index.html">
        <img src="$heroSrc" alt="Tots drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">Tots</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="tots/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing Tots card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added Tots card to Sketches index."
    }
    else {
        Write-Warning "Could not find an index/gallery grid on Sketches index. Tots page was built, but the main Sketches index was not updated."
    }
...
Write-Host ""
Write-Host "Done. Tots category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\tots\index.html"
Write-Host ""
Write-Host "Then click several individual Tots pages to check images and titles."
