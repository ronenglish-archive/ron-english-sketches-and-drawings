...
$CategoryTitle = 'Ungulates'
$CategorySlug = 'ungulates'

# The screenshots appear to show a possible nested folder:
# images\Sketches\Ungulates\Ungulates\[subcategory]
# but your typed paths say:
# images\Sketches\Ungulates\[subcategory]
# This script automatically uses whichever one exists.
$ImageRootDirect = Join-Path $RepoRoot 'images\Sketches\Ungulates'
$ImageRootNested = Join-Path $RepoRoot 'images\Sketches\Ungulates\Ungulates'

$HtmlRoot = Join-Path $RepoRoot "Sketches\$CategorySlug"
$SketchesIndexPath = Join-Path $RepoRoot 'Sketches\index.html'

$Subcategories = @(
    [pscustomobject]@{ Title='Camels'; Slug='camels'; ImageFolder='camels' },
    [pscustomobject]@{ Title='Cattle'; Slug='cattle'; ImageFolder='cattle' },
    [pscustomobject]@{ Title='Deer'; Slug='deer'; ImageFolder='deer' },
    [pscustomobject]@{ Title='Elephants'; Slug='elephants'; ImageFolder='Elephants' },
    [pscustomobject]@{ Title='Equines'; Slug='equines'; ImageFolder='Equines' },
    [pscustomobject]@{ Title='Rhinos'; Slug='rhinos'; ImageFolder='rhinos' },
    [pscustomobject]@{ Title='Sheep'; Slug='sheep'; ImageFolder='sheep' }
)

function Test-SubfoldersExist {
    param([Parameter(Mandatory=$true)][string]$Root)

    foreach ($sub in $Subcategories) {
        if (Test-Path -LiteralPath (Join-Path $Root $sub.ImageFolder)) {
            return $true
        }
    }
...
if (Test-SubfoldersExist -Root $ImageRootDirect) {
    $ImageRoot = $ImageRootDirect
    $ImageCategoryWebParts = @('Ungulates')
}
elseif (Test-SubfoldersExist -Root $ImageRootNested) {
    $ImageRoot = $ImageRootNested
    $ImageCategoryWebParts = @('Ungulates', 'Ungulates')
}
else {
    throw "Could not find Ungulates subfolders under either: $ImageRootDirect OR $ImageRootNested"
}
...
function New-WebImagePath {
    ...
    $encodedParts = @()
    foreach ($part in $ImageCategoryWebParts) {
        $encodedParts += Encode-Part $part
    }
    $encodedParts += Encode-Part $SubfolderName
    $encodedParts += Encode-Part $FileName

    return "$Prefix/" + ($encodedParts -join '/')
}
...
function Backup-FileIfExists {
    ...
    if (Test-Path -LiteralPath $Path) {
        ...
        $backupPath = Join-Path $backupDir "$name-before-ungulates-build-$timestamp$ext"
...
function Get-BaseTitleFromFileName {
    ...
    while ($changed) {
        ...
        # Remove common final view descriptors.
        ...
        $s = $s -replace '(?i)\s+rear$', ''
        $s = $s -replace '(?i)\s+back$', ''
        $s = $s -replace '(?i)\s+side$', ''
        $s = $s -replace '(?i)\s+head$', ''
        $s = $s -replace '(?i)\s+profile$', ''
        $s = $s -replace '(?i)\s+portrait$', ''
        $s = $s -replace '(?i)\s+turnaround$', ''
        $s = $s -replace '(?i)\s+sketch front$', ''
        $s = $s -replace '(?i)\s+sketch left$', ''
...
function Select-ThumbFile {
    ...
    if ($SubcategoryTitle -eq 'Camels') {
        $categoryPreferredPatterns = @('(?i)^Camel Family Icon COLOR', '(?i)^Camel Junior COLOR', '(?i)^Camel Sketch COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Cattle') {
        $categoryPreferredPatterns = @('(?i)^Cattle Family Icon COLOR', '(?i)^Cowgirl Icon COLOR', '(?i)^Cathy Cowgirl COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Deer') {
        $categoryPreferredPatterns = @('(?i)^Deer Family Icon COLOR', '(?i)^Bomba Deer.*COLOR', '(?i)^King Camo COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Elephants') {
        $categoryPreferredPatterns = @('(?i)^Elephant Family Icon COLOR 1', '(?i)^Elefanka front ORIGINAL COLOR', '(?i)^Trunk the Orange Elephant COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Equines') {
        $categoryPreferredPatterns = @('(?i)^Equines Family Icon COLOR', '(?i)^Unicorn Icon COLOR', '(?i)^Baby Horse COLOR')
    }
    elseif ($SubcategoryTitle -eq 'Rhinos') {
        $categoryPreferredPatterns = @('(?i)^Rainbow Rhino COLOR', '(?i)^Kaiju - rainbow Rhino')
    }
    elseif ($SubcategoryTitle -eq 'Sheep') {
        $categoryPreferredPatterns = @('(?i)^Falla Sheep With Gun COLOR', '(?i)^Hypno Sheep COLOR', '(?i)^Salvador Sheep COLOR')
    }
    ...
    $rules = @(
        { param($f) $f.Name -match '(?i)\bCOLOR\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear)\b' -and $f.Name -notmatch '(?i)colorway' },
        { param($f) $f.Name -match '(?i)\bORIGINAL COLOR\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear)\b' },
        { param($f) $f.Name -match '(?i)\bORIGINAL\b' -and $f.Name -notmatch '(?i)\b(side|back|tail|rear)\b' },
...
<head>
  ...
  <title>$($Group.Title) | $($Subcategory.Title) | Ungulates | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    <nav class="nav-pills" aria-label="Site navigation">
      ...
      <a class="nav-pill" href="../../index.html">Ungulates</a>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>$($Subcategory.Title) | Ungulates | Ron English Catalogue Raisonné</title>
...
$cardsHtml
    ...
    <div class="back-link">
      <a href="../index.html">&larr; Back to Ungulates</a>
    </div>
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Ungulates | Sketches &amp; Drawings | Ron English Catalogue Raisonné</title>
...
<body>
  <main class="wrap page-narrow">
    ...
    <header class="page-header">
      <h1 class="page-title">Ungulates</h1>
    </header>

    <section class="index-grid" aria-label="Ungulates subcategories">
$cardsHtml
    </section>
...
function Update-MainSketchesIndex {
    ...
    $preferred = @(
        [pscustomobject]@{ Sub='Equines'; Name='Equines Family Icon COLOR.jpg' },
        [pscustomobject]@{ Sub='Cattle'; Name='Cattle Family Icon COLOR.jpg' },
        [pscustomobject]@{ Sub='Elephants'; Name='Elephant Family Icon COLOR 1.jpg' },
        [pscustomobject]@{ Sub='Camels'; Name='Camel Family Icon COLOR.jpg' }
    )
    ...
    $encodedParts = @()
    foreach ($part in $ImageCategoryWebParts) { $encodedParts += Encode-Part $part }
    $encodedParts += Encode-Part $heroSubcategory.ImageFolder
    $encodedParts += Encode-Part $heroFile.Name
    $heroSrc = '../images/Sketches/' + ($encodedParts -join '/')
    ...
    $newCard = @"
      <a class="gallery-item" href="ungulates/index.html">
        <img src="$heroSrc" alt="Ungulates drawings by Ron English" loading="lazy" />
        <div class="entry-body">
          <p class="work-title">Ungulates</p>
        </div>
      </a>
"@

    $cardPattern = '(?is)<a\b(?=[^>]*\bclass="[^"]*\bgallery-item\b[^"]*")[^>]*href="ungulates/index\.html"[^>]*>.*?</a>'

    if ($html -match $cardPattern) {
        $html = [regex]::Replace($html, $cardPattern, $newCard, 1)
        Write-Host "Updated existing Ungulates card on Sketches index."
    }
    elseif ($html -match '(?is)(<section\b[^>]*class="[^"]*(?:index-grid|gallery-grid)[^"]*"[^>]*>)(.*?)(</section>)') {
        ...
        Write-Host "Added Ungulates card to Sketches index."
    }
    else {
...
Write-Host "Using image root:"
Write-Host "  $ImageRoot"
...
Write-Host ""
Write-Host "Done. Ungulates category built."
Write-Host "Check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\index.html"
Write-Host ""
Write-Host "Then check:"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\camels\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\cattle\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\deer\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\elephants\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\equines\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\rhinos\index.html"
Write-Host "  D:\github\ron-english-sketches-and-drawings\Sketches\ungulates\sheep\index.html"
Write-Host ""
Write-Host "The script also tried to add/update the Ungulates card on:"
